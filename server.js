...
const {startCollector,status:collectorStatus,databaseStatus,databaseHistory,analysisStatus,markets}=require('./collector');
...
app.get('/markets',(req,res)=>res.json({ok:true,markets:markets()}));
app.get('/market-analysis',async(req,res)=>{const symbol=String(req.query.symbol||'');if(!symbol)return res.status(400).json({ok:false,error:'symbol is required'});res.json(analysisStatus(symbol));});
