...
function connect(){if(socket)try{socket.close()}catch(_){};socket=new WebSocket(DERIV_WS);socket.on('open',()=>{connected=true;reconnectDelay=2000;lastError=null;console.log('Background Deriv collector connected via legacy public market-data endpoint');socket.send(JSON.stringify({active_symbols:'brief',product_type:'basic',req_id:1}))});socket.on('message',raw=>{let m;try{m=JSON.parse(raw.toString())}catch(_){return}if(m.error){lastError=m.error.message;console.error('Deriv collector error:',JSON.stringify(m.error));return}if(diagnosticMessages<30&&m.msg_type!=='active_symbols'){diagnosticMessages++;console.log('Deriv collector message:',JSON.stringify({msg_type:m.msg_type,error:m.error,echo_req:m.echo_req,subscription:m.subscription,tick:m.tick?{symbol:m.tick.symbol,epoch:m.tick.epoch,quote:m.tick.quote}:undefined,history:m.history?{count:m.history.times?.length}:undefined}));}if(m.msg_type==='active_symbols'){const list=(m.active_symbols||[]).filter(x=>classify(x.underlying_symbol_name||x.display_name));marketCatalog=list.map(x=>({symbol:x.underlying_symbol||x.symbol,name:x.underlying_symbol_name||x.display_name||x.underlying_symbol||x.symbol,group:classify(x.underlying_symbol_name||x.display_name)})).filter(x=>x.symbol);marketCount=marketCatalog.length;console.log('Background collector markets:',marketCount);console.log('Background collector first market:',JSON.stringify(list[0]));for(const x of list){const symbol=x.underlying_symbol||x.symbol;if(symbol)socket.send(JSON.stringify({ticks:symbol,subscribe:1}))}return}if(m.msg_type==='history'&&m.history&&Array.isArray(m.history.times)&&Array.isArray(m.history.prices)){const symbol=String(m.echo_req?.ticks_history||m.echo_req?.symbol||'');if(symbol){const ps=pipSizeBySymbol.get(symbol);for(let i=0;i<m.history.times.length;i++){const epoch=Number(m.history.times[i]),quote=Number(m.history.prices[i]);if(!Number.isFinite(epoch)||!Number.isFinite(quote)||lastEpochBySymbol.get(symbol)===epoch)continue;lastEpochBySymbol.set(symbol,epoch);const digit=lastDigit(quote,ps);queue.push({symbol,epoch,quote,digit});addAnalysisTick(symbol,digit);totalReceived++;lastTickAt=new Date().toISOString();}console.log('Background history loaded:',symbol,m.history.times.length);if(queue.length>MAX_QUEUE)queue=queue.slice(-MAX_QUEUE);}return}if(m.msg_type==='tick'&&m.tick){const symbol=m.tick.symbol||m.tick.underlying_symbol;if(!symbol)return;const epoch=Number(m.tick.epoch||Math.floor(Date.now()/1000));if(lastEpochBySymbol.get(symbol)===epoch)return;lastEpochBySymbol.set(symbol,epoch);const quote=Number(m.tick.quote);if(!Number.isFinite(quote))return;const ps=Number.isFinite(Number(m.tick.pip_size))?Number(m.tick.pip_size):pipSizeBySymbol.get(symbol);const digit=lastDigit(quote,ps);queue.push({symbol,epoch,quote,digit});addAnalysisTick(symbol,digit);if(queue.length>MAX_QUEUE)queue=queue.slice(-MAX_QUEUE);totalReceived++;lastTickAt=new Date().toISOString();if(totalReceived<=5||totalReceived%100===0)console.log('Background tick received:',symbol,'digit',digit,'total',totalReceived)}});socket.on('error',err=>{lastError=err.message;console.error('Background collector socket error:',err.message)});socket.on('close',()=>{connected=false;console.log('Background collector disconnected; reconnecting');scheduleReconnect()})}
