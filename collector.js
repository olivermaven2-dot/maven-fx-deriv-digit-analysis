...
let pool=null,socket=null,reconnectTimer=null,reconnectDelay=2000,connected=false,marketCount=0,queue=[],lastTickAt=null,lastDbWriteAt=null,lastError=null,totalReceived=0,totalStored=0,marketCatalog=[],diagnosticMessages=0;
const lastEpochBySymbol=new Map();
const pipSizeBySymbol=new Map();
...
function classify(...values){const n=values.filter(Boolean).map(v=>String(v).toLowerCase()).join(' ');if(n.includes('jump')||/\bjd\d+\b/.test(n)||n.includes('jump_'))return'Jump Indices';if(n.includes('volatility')||/\br_\d+\b/.test(n)||/\b1hz\d+v\b/.test(n)){return n.includes('1s')||n.includes('1-second')||n.includes('1hz')?'Volatility 1s':'Volatility Indices'}return null}
...
let requestId=100;const historyRequests=new Map();
function connect(){if(socket)try{socket.close()}catch(_){};socket=new WebSocket(DERIV_WS);socket.on('open',()=>{connected=true;reconnectDelay=2000;lastError=null;console.log('Background Deriv collector connected via public market-data endpoint');socket.send(JSON.stringify({active_symbols:'brief',product_type:'basic',req_id:1}))});socket.on('message',raw=>{let m;try{m=JSON.parse(raw.toString())}catch(_){return}if(m.error){lastError=m.error.message;console.error('Deriv collector error:',JSON.stringify(m.error));return}if(diagnosticMessages<30&&m.msg_type!=='active_symbols'){diagnosticMessages++;console.log('Deriv collector message:',JSON.stringify({msg_type:m.msg_type,error:m.error,echo_req:m.echo_req,subscription:m.subscription,tick:m.tick?{symbol:m.tick.symbol,epoch:m.tick.epoch,quote:m.tick.quote}:undefined,history:m.history?{count:m.history.times?.length}:undefined}));}if(m.msg_type==='active_symbols'){const raw=Array.isArray(m.active_symbols)?m.active_symbols:[];console.log('Background active_symbols payload:',JSON.stringify({count:raw.length,keys:raw[0]?Object.keys(raw[0]):[],sample:raw[0]||null}));const list=raw.map(x=>{const symbol=x.underlying_symbol||x.symbol;const name=x.underlying_symbol_name||x.display_name||x.market_display_name||symbol;const group=classify(name,symbol,x.market,x.submarket,x.subgroup,x.underlying_symbol_type,x.symbol_type);if(symbol&&x.pip_size!=null)pipSizeBySymbol.set(symbol,Number(x.pip_size));else if(symbol&&x.pip!=null)pipSizeBySymbol.set(symbol,Math.max(0,Math.round(-Math.log10(Number(x.pip)))));return {raw:x,symbol,name,group}}).filter(x=>x.symbol&&x.group);marketCatalog=list.map(x=>({symbol:x.symbol,name:x.name,group:x.group}));marketCount=marketCatalog.length;console.log('Background collector markets:',marketCount);console.log('Background collector first market:',JSON.stringify(list[0]?.raw||null));for(const x of list){const symbol=x.symbol;if(!symbol)continue;const reqId=++requestId;historyRequests.set(reqId,symbol);socket.send(JSON.stringify({ticks_history:symbol,count:500,end:'latest',style:'ticks',req_id:reqId}));socket.send(JSON.stringify({ticks:symbol,subscribe:1,req_id:++requestId}))}return}if(m.msg_type==='history'&&m.history&&Array.isArray(m.history.times)&&Array.isArray(m.history.prices)){const reqId=Number(m.req_id||m.echo_req?.req_id);const symbol=historyRequests.get(reqId)||String(m.echo_req?.ticks_history||m.echo_req?.symbol||'');if(symbol){historyRequests.delete(reqId);const ps=pipSizeBySymbol.get(symbol);for(let i=0;i<m.history.times.length;i++){const epoch=Number(m.history.times[i]),quote=Number(m.history.prices[i]);if(!Number.isFinite(epoch)||!Number.isFinite(quote)||lastEpochBySymbol.get(symbol)===epoch)continue;lastEpochBySymbol.set(symbol,epoch);const digit=lastDigit(quote,ps);queue.push({symbol,epoch,quote,digit});addAnalysisTick(symbol,digit);totalReceived++;lastTickAt=new Date().toISOString();}console.log('Background history loaded:',symbol,m.history.times.length);if(queue.length>MAX_QUEUE)queue=queue.slice(-MAX_QUEUE);}return}if(m.msg_type==='tick'&&m.tick){const symbol=m.tick.symbol||m.tick.underlying_symbol;if(!symbol)return;const epoch=Number(m.tick.epoch||Math.floor(Date.now()/1000));if(lastEpochBySymbol.get(symbol)===epoch)return;lastEpochBySymbol.set(symbol,epoch);const quote=Number(m.tick.quote);if(!Number.isFinite(quote))return;const ps=Number.isFinite(Number(m.tick.pip_size))?Number(m.tick.pip_size):pipSizeBySymbol.get(symbol);const digit=lastDigit(quote,ps);queue.push({symbol,epoch,quote,digit});addAnalysisTick(symbol,digit);if(queue.length>MAX_QUEUE)queue=queue.slice(-MAX_QUEUE);totalReceived++;lastTickAt=new Date().toISOString();if(totalReceived<=5||totalReceived%100===0)console.log('Background tick received:',symbol,'digit',digit,'total',totalReceived)}});socket.on('error',err=>{lastError=err.message;console.error('Background collector socket error:',err.message)});socket.on('close',()=>{connected=false;console.log('Background collector disconnected; reconnecting');scheduleReconnect()})}
...
function markets(){return marketCatalog.slice().sort((a,b)=>(a.group+a.name).localeCompare(b.group+b.name))}
...
module.exports={startCollector,status,databaseStatus,databaseHistory,analysisStatus,markets};